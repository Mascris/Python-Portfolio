...
def buy_items():
    try:
        ...
        amounts = int(input(f"Enter how many pieces of '{item}' you want to buy: "))
        if amounts <= 0: # Changed to <= 0 for robustness
            print("You should at least buy one item.")
            return

        player_id, player_coins = get_players_data(for_buy=True) # Now correctly retrieves player_id
        if player_id is None: # Check if a valid player ID was retrieved
            print("Cannot proceed without selecting a valid player ID!")
            return
        
        # Start connection and cursor using 'with' statement for resource management
        with connect() as conn:
            with conn.cursor() as cursor:
                # 1. Check if item exists in store and has enough quantity
                # Select item_id along with other details
                cursor.execute("SELECT item_id, name, price, quantity FROM store WHERE LOWER(name) = %s AND quantity >= %s", (item, amounts,))
                store_item_data = cursor.fetchone() # Renamed result for clarity

                if not store_item_data:
                    print(f"Invalid item name ('{item}') or not enough quantity available in store!")
                    return # Exit if item not found or not enough stock

                # Extract data from the fetched store_item_data
                store_item_id = store_item_data[0] # Assuming item_id is the first column
                store_item_name = store_item_data[1] # For printing confirmation
                store_item_price = store_item_data[2] # For calculating total price
                # store_item_quantity_available = store_item_data[3] # Not strictly needed here, but good to know

                total_price = store_item_price * amounts

                # 2. Check player's coin balance
                ...
                # --- TRANSACTION BEGINS (implicitly with the first UPDATE, committed at conn.commit()) ---
                # 3. Deduct coins from player
                # Corrected table name to 'players' and column to 'coins', and target 'player_id'
                cursor.execute("UPDATE players SET coins = coins - %s WHERE player_id = %s", (total_price, player_id))
                
                # 4. Reduce quantity in store
                # Corrected table name to 'store' and column to 'quantity'
                cursor.execute("UPDATE store SET quantity = quantity - %s WHERE item_id = %s", (amounts, store_item_id))

                # 5. Add/Update item in player's inventory
                ...
                # --- TRANSACTION ENDS ---
                conn.commit() # Commit all changes if everything above succeeded
                print(f"🥳 You successfully bought {amounts}x '{store_item_name}' for {total_price} coins!")
                # Optional: Re-fetch player_coins to show new balance or calculate locally:
                print(f"Your new balance is {player_coins - total_price} coins.")
                
        # The original code returned 'result' which was the store_item_data.
        # It's better to return a success indicator or the purchased item data if needed by caller.
        return store_item_data

    except ValueError:
        print("⚠️ Invalid input. Please ensure you enter a whole number for the amount.")
    except Exception as e:
        # The 'with connect() as conn' block generally handles rollback on exception automatically
